...
struct Treap {
	struct Node {
		...
		Node() : l(0), r(0), p(0), y(rng()), size(1), mark(0), sub_mark(0) {
			if (!rng_init)
				rng =
						mt19937(std::chrono::duration_cast<std::chrono::nanoseconds>(
	...
	static unsigned int size(Node* n) { return n ? n->size : 0; }
...
struct EulerTourTree {
	...
	EulerTourTree(int n, int m, vector<tuple<int, int, bool> >& _graph)
			: graph(_graph), edges(2 * m), first_edge(n) {
		for (int i = 0; i < n; ++i) {
			first_edge[i] = Treap::Node();
		}
	}
	bool is_single(int a) { return first_edge[a].size == 0 && !first_edge[a].p; }
	bool connected(int a, int b) {
		return Treap::root(&first_edge[a]) == Treap::root(&first_edge[b]);
	}
	void reroot(int a) { Treap::evert(&first_edge[a]); }
	...
	void cut(int edge_index) {
		Treap::Node *e_ab = &edges[2 * edge_index],
								*e_ba = &edges[2 * edge_index | 1];
	...
	void set_edge_mark(Treap::Node* c, char mark) {
		if ((c - edges.data()) % 2)
			return set_edge_mark((c - edges.data()) / 2, mark);
		c->mark = mark;
		Treap::re_path(c);
	}
	...
	unsigned int index_edge(Treap::Node* n) { return (n - edges.data()) / 2; }
	unsigned int index_vertex(Treap::Node* n) { return n - first_edge.data(); }
};

struct LayerStructure {
	int logn, n, m;
	vector<EulerTourTree> levels;
	vector<tuple<int, int, bool> > edges;
	vector<unsigned int> free_edges;
	// stores all edge and their index
	vector<map<int, int> > graph;
	LayerStructure(int _n, int _m) : n(_n), m(_m) {
	...
	int calc_graph_pos(int level, int vertex) { return level * n + vertex; }
	bool connected(int a, int b) { return levels[0].connected(a, b); }
	...
	void cut(int a, int b) {
		...
		while (graph[calc_graph_pos(level, a)].find(b) ==
					 graph[calc_graph_pos(level, a)].end()) {
			--level;
		}
		...
		for (int i = level; i >= 0; --i) {
			...
			if (levels[i].call_on_nodes<1>(
							small, [this, a, b, i, small, edge_index](Treap::Node* c) {
